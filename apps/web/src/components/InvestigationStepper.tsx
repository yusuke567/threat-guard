...
export default function InvestigationStepper({ threat, contentAnalysis, latestAnalysis, probing, probeStatus, onProbe, onStartTakedown, takedownStep, takedownError, takedownSuccess, registrar, abuseEmail, onAbuseEmailChange, onGenerate, template, onTemplateChange, onSend, onResetFlow, takedownPdfUrl }: StepperProps) {
  ...
  return (
    <Card>
      <h2 className="text-lg font-bold text-[var(--text-primary)] mb-5 flex items-center gap-2"><Icon name="search" size={20} /> この脅威の調査状況</h2>

      <div className="space-y-0">
        {/* STEP 1: ドメイン分析 */}
        <div className="relative pl-8 pb-6">
          {/* Connector line */}
          <div className="absolute left-3 top-6 bottom-0 w-0.5 bg-surface-elevated" />
          {/* Step icon */}
          ...
          <div className="mb-2">
            <span className="text-sm font-bold text-[var(--text-primary)]">STEP 1: ドメイン分析</span>
            <span className="text-xs text-[var(--text-tertiary)] ml-2">ドメイン名・登録情報をAIが分析しました</span>
          </div>
          ...
          {latestAnalysis && (
            <details className="mt-3">
              ...
              <div className="mt-3 rounded-lg border border-[var(--border-default)] p-3">
                <h4 className="text-xs font-bold text-[var(--text-primary)] mb-2">AI分析結果</h4>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-[var(--text-tertiary)]">（判定確度: {Math.round(latestAnalysis.confidence * 100)}%）</span>
                </div>
                <p className="text-xs text-[var(--text-secondary)] bg-surface-base rounded-lg p-3">
        ...
        <div className="relative pl-8 pb-6">
          ...
          <div className={`absolute left-0 top-0.5 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${
            step2Done ? 'bg-green-500 text-white' : currentStep === 2 ? 'bg-blue-500 text-white' : 'bg-surface-elevated text-[var(--text-secondary)]'
          }`}>
            {step2Done ? '✓' : '2'}
          </div>

          <div className="mb-2">
            <span className="text-sm font-bold text-[var(--text-primary)]">STEP 2: <GlossaryTerm term="プローブ">サイト調査</GlossaryTerm></span>
            <span className="text-xs text-[var(--text-tertiary)] ml-2">実際にサイトにアクセスして内容を調べます</span>
          </div>
          ...
          {step2Done && siteResult ? (
            <>
              <div className={`rounded-lg border p-3 ${colorMap[siteResult.color]}`}>
                ...
                <p className={`text-xs font-medium mt-1.5 flex items-center gap-1 ${textColorMap[siteResult.color]}`}><Icon name="lightbulb" size={14} /> {siteResult.action}</p>
              </div>
              ...
              <div className="text-xs text-[var(--text-secondary)] mt-2 space-y-1">
                ...
                <div className="text-[var(--text-tertiary)]">最終調査: {new Date(probe.probeAt).toLocaleString('ja-JP')}</div>
              </div>

              <Button variant="ghost" size="sm" onClick={onProbe} disabled={probing} className="mt-2">
                {probeStatus === 'running' ? '⏳ 再調査中...' : <><Icon name="refresh" size={14} /> 再調査する</>}
              </Button>
                ...
                <div className="mt-3 space-y-3">
                  ...
                  {contentAnalysis && (
                    <div className="rounded-lg border border-[var(--border-default)] p-3">
                      <h4 className="text-xs font-bold text-[var(--text-primary)] mb-2">危険な兆候チェック</h4>
                      <div className="space-y-2 text-xs">
                        <div className="flex items-center gap-2">
                          <span>{contentAnalysis.hasLoginForm ? '⚠️' : '✅'}</span>
                          <span className={contentAnalysis.hasLoginForm ? 'text-red-700 dark:text-red-300' : 'text-[var(--text-secondary)]'}>
                            ログイン画面の模倣: {contentAnalysis.hasLoginForm ? '検出されました' : '検出なし'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span>{contentAnalysis.hasPasswordField ? '⚠️' : '✅'}</span>
                          <span className={contentAnalysis.hasPasswordField ? 'text-red-700 dark:text-red-300' : 'text-[var(--text-secondary)]'}>
                            パスワード入力欄: {contentAnalysis.hasPasswordField ? '検出されました' : '検出なし'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span>{contentAnalysis.logoDetected ? '⚠️' : '✅'}</span>
                          <span className={contentAnalysis.logoDetected ? 'text-amber-700' : 'text-[var(--text-secondary)]'}>
                        ...
                        {contentAnalysis.imageSimilarity !== null && contentAnalysis.imageSimilarity !== undefined && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className={contentAnalysis.imageSimilarity > 0.7 ? 'text-red-700 dark:text-red-300' : 'text-[var(--text-secondary)]'}>
        ...
        <div className="relative pl-8">
          {/* Step icon */}
          <div className={`absolute left-0 top-0.5 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${
            step3Done ? 'bg-green-500 text-white' : shouldRecommendTakedown ? 'bg-red-500 text-white animate-pulse' : 'bg-surface-elevated text-[var(--text-secondary)]'
          }`}>
            {step3Done ? '✓' : '3'}
          </div>

          <div className="mb-2">
            <span className="text-sm font-bold text-[var(--text-primary)]">STEP 3: 削除申請</span>
            <span className="text-xs text-[var(--text-tertiary)] ml-2">ドメインの停止を<GlossaryTerm term="レジストラ">レジストラ</GlossaryTerm>に依頼します</span>
          </div>

          {/* Error / Success */}
          {takedownError && (
            <Alert variant="error" className="mb-2 !p-2 text-xs">{takedownError}</Alert>
          )}
          {takedownSuccess && (
            <Alert variant="success" className="mb-2 !p-2 text-xs">✅ {takedownSuccess}</Alert>
          )}
          ...
          {step3Done ? (
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              <span className="text-xs text-[var(--text-secondary)]">WHOIS情報からabuse連絡先を取得中...</span>
            </div>
            ...
            <div className="space-y-3 border border-blue-200 dark:border-blue-800 rounded-lg p-3 bg-blue-50 dark:bg-blue-900/30/50">
              <div className="text-xs text-[var(--text-secondary)]">
                <GlossaryTerm term="レジストラ">レジストラ</GlossaryTerm>: <span className="font-medium text-[var(--text-primary)]">{registrar}</span>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 border border-[var(--border-default)] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={onGenerate}>📝 削除申請文面を生成</Button>
                <Button variant="ghost" size="sm" onClick={onResetFlow}>キャンセル</Button>
              </div>
            </div>
          ) : takedownStep === 'generating' ? (
            <div className="flex items-center gap-3 py-2">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" />
              <span className="text-xs text-[var(--text-secondary)]">AIが削除申請文面を生成中...（30秒ほど）</span>
            </div>
          ) : takedownStep === 'review' ? (
            <div className="space-y-3">
              <div className="border border-amber-200 rounded-lg p-3 bg-amber-50/50">
                ...
                <div className="grid grid-cols-2 gap-1 text-[10px] text-[var(--text-secondary)] mb-2">
                  <div>送信先: <span className="font-medium text-[var(--text-primary)]">{abuseEmail}</span></div>
                  <div>レジストラ: <span className="font-medium text-[var(--text-primary)]">{registrar}</span></div>
                </div>
                <textarea
                  value={template || ''}
                  onChange={(e) => onTemplateChange?.(e.target.value)}
                  rows={10}
                  className="w-full px-2 py-1.5 border border-[var(--border-default)] rounded-lg text-[10px] font-mono focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div className="flex gap-2">
                <Button variant="danger" size="sm" onClick={onSend}>✅ 確認して送信</Button>
                {takedownPdfUrl && <a href={takedownPdfUrl} target="_blank" className="px-3 py-1.5 bg-surface-elevated text-[var(--text-primary)] rounded-lg text-xs">📄 PDF</a>}
                <Button variant="ghost" size="sm" onClick={onResetFlow}>キャンセル</Button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 py-2">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" />
              <span className="text-xs text-[var(--text-secondary)]">メールを送信中...</span>
            </div>
          ) : takedownStep === 'sent' ? (
            <div className="space-y-2">
              <Button variant="ghost" size="sm" onClick={onResetFlow}>新しい削除申請を作成</Button>
            </div>
          ) : (
            <div className={`rounded-lg border p-3 ${shouldRecommendTakedown ? 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/30' : 'border-[var(--border-default)] bg-surface-base'}`}>
              {shouldRecommendTakedown ? (
                <>
                  <p className="text-xs text-red-700 dark:text-red-300 mb-2 font-medium">⚡ 削除申請が推奨されています</p>
                  <Button variant="danger" onClick={onStartTakedown}>
                    <Icon name="alertTriangle" size={16} /> 削除申請を開始
                  </Button>
                </>
              ) : (
                <p className="text-xs text-[var(--text-secondary)]">STEP 1・2 の結果に基づいて、削除申請が必要か判断します</p>
              )}
            </div>
          )}
            ...
            <details className="mt-3">
              ...
              <div className="mt-2 space-y-2">
                {threat.takedowns.map((td: any) => (
                  <div key={td.id} className="border border-[var(--border-default)] rounded-lg p-3">
                    ...
                    <div className="text-[10px] text-[var(--text-tertiary)]">{new Date(td.createdAt).toLocaleString('ja-JP')}</div>
                    <pre className="text-[10px] text-[var(--text-secondary)] bg-surface-base rounded p-2 overflow-x-auto whitespace-pre-wrap max-h-32 mt-1">{td.template}</pre>
                  </div>
                ))}
              </div>
            </details>
