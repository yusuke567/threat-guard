...
import { PageHeader, Button, Card, useToast } from '@/components/ui';
...
export default function TakedownsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="削除申請の進捗" description="送信済みの削除申請のステータスを管理します" />
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <Card className="!p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-[var(--text-primary)] mb-1">削除申請はまだありません</h3>
            <p className="text-[var(--text-secondary)] text-sm mb-4">脅威一覧から脅威を選択して削除申請を送信してください。</p>
            <Button
              onClick={() => router.push('/')}
            >
              脅威一覧へ
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-[var(--text-primary)] flex items-center gap-2">
                      {group.recipientType === 'police' ? '🚔' : group.recipientType === 'hosting' ? '🖥️' : '📧'} {group.recipientName || group.registrar}
                      <span className="text-sm font-normal text-[var(--text-secondary)]">— {group.items.length}件</span>
                  ...
                  <div key={item.id} className="px-4 sm:px-6 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-[var(--text-tertiary)]">{item.brandName}</div>
                      </div>
                    </div>
                      ...
                      {item.status === 'rejected' && (
                        <button
                          onClick={() => {
                            ...
                            toast.info('再申請機能: 脅威一覧から再度選択して申請してください。');
                          }}
