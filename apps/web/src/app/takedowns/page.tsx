...
export default function TakedownsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">削除申請の進捗</h1>
        <p className="text-gray-500 mt-1">送信済みの削除申請のステータスを管理します</p>
      </div>
      ...
      {data?.summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            { key: 'total', label: '全件', color: 'bg-gray-50 border-gray-200', textColor: 'text-gray-900' },
            { key: 'sent', label: '送信済み', color: 'bg-blue-50 border-blue-200', textColor: 'text-blue-700' },
            { key: 'awaiting_response', label: '回答待ち', color: 'bg-yellow-50 border-yellow-200', textColor: 'text-yellow-700' },
            { key: 'completed', label: '削除完了', color: 'bg-green-50 border-green-200', textColor: 'text-green-700' },
            { key: 'rejected', label: '却下', color: 'bg-red-50 border-red-200', textColor: 'text-red-700' },
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                      📧 {group.registrar}
                      <span className="text-sm font-normal text-gray-500">— {group.items.length}件</span>
                    </h3>
                  ...
                  <div key={item.id} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className={`text-xs font-medium rounded-full px-3 py-1 border-0 cursor-pointer ${statusColors[item.status] || 'bg-gray-100'}`}
                      >
