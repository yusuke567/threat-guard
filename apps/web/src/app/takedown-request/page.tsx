...
export default function TakedownRequestPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">削除申請を送信しました</h1>
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {/* Header + Progress */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">削除申請</h1>
        <p className="text-gray-500 mt-1">選択した脅威の管理会社に削除を依頼します</p>
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1">申請対象: {activeThreats.length}件</h2>
            <p className="text-sm text-gray-500 mb-4">削除を申請する脅威を確認してください。不要な場合は✕で除外できます。</p>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-4 p-3 rounded-lg border ${
                      excluded ? 'opacity-40 bg-gray-50 border-gray-100' : 'bg-white border-gray-200'
                    }`}
                  >
                    <button
                      onClick={() => {
                      ...
                      className="text-gray-400 hover:text-red-500 text-lg font-bold w-6"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500">{t.categoryDescription}</div>
                    </div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${risk.color}`}>
                      {risk.label}
                    </span>
                    <div className="text-right text-xs text-gray-500 min-w-[180px]">
                      <div>→ {t.registrar}</div>
          ...
          {/* 送信先まとめ */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="font-bold text-gray-900 mb-3">送信先まとめ</h3>
            {activeGroups.map((g, i) => (
              <div key={i} className="flex items-center gap-3 py-2">
                <span>📧</span>
                <span className="font-medium text-sm">{g.registrar}</span>
                {g.abuseEmail ? (
                  <span className="text-xs text-gray-500">({g.abuseEmail})</span>
                ) : (
                  <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">— {g.threats.filter((t) => !excludedIds.has(t.threatId)).length}件</span>
              </div>
            ))}
          </div>
          ...
          {activeGroups.map((g, i) => {
            ...
            return (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                      📧 {g.registrar}
                      <span className="text-sm font-normal text-gray-500">({g.threats.length}件)</span>
                    </h3>
                    <p className="text-xs text-gray-500">送信先: {email || '未設定'}</p>
                  </div>
                ...
                {/* Evidence */}
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">添付エビデンス:</h4>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">以下の内容で削除申請を送信します</h2>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 text-sm text-gray-700 font-medium">
