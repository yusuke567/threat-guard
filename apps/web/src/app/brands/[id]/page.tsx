...
export default function BrandDetailPage() {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.warning('対応形式: PNG, JPG, GIF, SVG, WebP');
      e.target.value = '';
      return;
    }
    ...
    if (file.size > LOGO_MAX_SIZE_MB * 1024 * 1024) {
      toast.warning(`ファイルサイズは${LOGO_MAX_SIZE_MB}MB以下にしてください。`);
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      toast.error(err.message || 'ロゴのアップロードに失敗しました');
    } finally {
  ...
  const handleTrademarkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err.message || '商標登録証明のアップロードに失敗しました');
    } finally {
  ...
  const handleAddDomain = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || 'ドメインの追加に失敗しました');
    } finally {
  ...
  const handleCsvImport = async () => {
    ...
    try {
      ...
      if (result.duplicateMessage) {
        toast.warning(result.duplicateMessage);
      }
      setCsvText('');
      setCsvMode(false);
      loadData();
    } catch (err: any) {
      toast.error(err.message || 'CSVインポートに失敗しました');
    } finally {
  ...
  const handleEmailSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || 'メール設定の保存に失敗しました');
    } finally {
  ...
  if (!brand) {
    return (
      <div className="text-center py-20">
        <p className="text-[var(--text-secondary)]">ブランドが見つかりません</p>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Brand Info Card */}
      <div id="section-logo" className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-6">
          ...
          <div className="flex-shrink-0">
            ...
            {brand.logoUrl && (
              <Button variant="ghost" size="sm" onClick={handleLogoDelete} className="text-red-500 hover:text-red-600 mt-1 w-full text-center">
                削除
              </Button>
            )}
            {logoUploading && <p className="text-xs text-gray-400 mt-1 text-center">アップロード中...</p>}
            <p className="text-[10px] text-[var(--text-tertiary)] mt-1 text-center leading-tight">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-[var(--text-secondary)] font-mono text-sm mt-1">{brand.domain}</p>
                {brand.organization && (
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={startEdit} className="bg-surface-elevated hover:bg-gray-200 dark:hover:bg-gray-600">
                  ✏️ 編集
                </Button>
                <Button variant="ghost" size="sm" onClick={handleDelete} className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30">
                  削除
                </Button>
              </div>
            </div>
            ...
            {brand.keywords && brand.keywords.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-3">
                {brand.keywords.split(',').filter(Boolean).map((kw) => (
                  <span key={kw} className="px-2 py-0.5 bg-surface-elevated rounded text-xs text-[var(--text-secondary)]">
                    {kw.trim()}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              <Button
                ...
                disabled={scanning}
                className="bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 hover:bg-purple-100"
              >
                🔤 類似ドメインスキャン
              </Button>
            </div>
      ...
      {(() => {
        ...
        return (
          <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-[var(--text-primary)]">🚀 セットアップ</h2>
              <span className="text-xs text-[var(--text-secondary)]">{doneCount}/{checks.length} 完了</span>
            </div>
            ...
            <div className="space-y-2">
              {checks.map(c => (
                <div key={c.key} className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${c.done ? 'bg-green-50 dark:bg-green-900/10' : 'bg-gray-50 dark:bg-gray-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/10'}`}>
                  ...
                  <span className={`text-sm ${c.done ? 'text-[var(--text-tertiary)] line-through' : 'text-[var(--text-primary)]'}`}>
                    {c.label}
                  </span>
                  {!c.done && (
      ...
      {editing && (
        <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
          <form onSubmit={handleUpdate} className="space-y-4">
            <h2 className="text-lg font-bold text-[var(--text-primary)]">ブランド編集</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">ブランド名</label>
                <input type="text" required className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">ドメイン</label>
                <input type="text" required className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]" value={editForm.domain} onChange={(e) => setEditForm({ ...editForm, domain: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">検知キーワード（カンマ区切り）</label>
              <input type="text" className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]" value={editForm.keywords} onChange={(e) => setEditForm({ ...editForm, keywords: e.target.value })} />
            </div>
            <p className="text-xs text-[var(--text-tertiary)]">保有ドメイン・メール送信設定はこのページ下部の専用セクションで管理できます。</p>

            <div className="flex gap-2">
              <Button variant="primary" type="submit">保存</Button>
              <Button variant="secondary" type="button" onClick={() => setEditing(false)}>キャンセル</Button>
            </div>
          </form>
        </div>
      )}
      ...
      {stats && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-4">
              <p className="text-xs text-[var(--text-secondary)]">検知数合計</p>
              <p className="text-2xl font-bold text-[var(--text-primary)] mt-1">{stats.totalThreats}</p>
            </div>
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-4">
              <p className="text-xs text-[var(--text-secondary)]">平均リスクスコア</p>
              <p className="text-2xl font-bold text-[var(--text-primary)] mt-1">{stats.averageRiskScore ?? '-'}</p>
            </div>
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-4">
              <p className="text-xs text-[var(--text-secondary)]">フィッシング確認</p>
              <p className="text-2xl font-bold text-red-600 mt-1">{stats.statusBreakdown.confirmed_phishing || 0}</p>
            </div>
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-4">
              <p className="text-xs text-[var(--text-secondary)]">削除完了</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Status */}
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
              <h2 className="text-sm font-bold text-[var(--text-primary)] mb-4">ステータス別内訳</h2>
              <div className="space-y-2">
                {Object.entries(stats.statusBreakdown).map(([status, count]) => {
                  ...
                  return (
                    <div key={status} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-[var(--text-secondary)] w-12 text-right">{count}</span>
                    </div>
                  );
                })}
            ...
            {/* Risk Distribution */}
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
              <h2 className="text-sm font-bold text-[var(--text-primary)] mb-4">リスク帯分布</h2>
              <div className="space-y-2">
                {['critical', 'high', 'medium', 'low', 'unknown'].map((band) => {
                  ...
                  return (
                    <div key={band} className="flex items-center gap-3">
                      <span className="text-xs w-24 text-[var(--text-secondary)]">{RISK_LABELS[band]}</span>
                      <div className="flex-1 bg-surface-elevated rounded-full h-2">
          ...
          {stats.dailyTrend.length > 0 && (
            <div className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
              <h2 className="text-sm font-bold text-[var(--text-primary)] mb-4">直近30日の検知推移</h2>
      ...
      <div id="section-domains" className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
        ...
        {(() => {
          ...
          return (
            <>
              <div className="mb-4">
                <h3 className="text-xs font-medium text-[var(--text-secondary)] mb-2 uppercase tracking-wide">🎯 プライマリドメイン（監視対象）</h3>
                    ...
                    {primaryDomains.map((d) => (
                      <div key={d.id} className="flex items-center justify-between px-3 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-mono text-[var(--text-primary)]">{d.domain}</span>
                        </div>
                        <Button variant="ghost" size="sm" onClick={() => handleRemoveDomain(d.id)} className="text-red-500 hover:text-red-600">✕</Button>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <div className="mb-4">
                <h3 className="text-xs font-medium text-[var(--text-secondary)] mb-2 uppercase tracking-wide">
                  🏢 保有ドメイン（ホワイトリスト）
                  ...
                  <div className="space-y-1">
                    {visibleOwned.map((d) => (
                      <div key={d.id} className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <div className="flex items-center gap-2">
                          <span className="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 text-[var(--text-secondary)] rounded text-[10px] font-medium">OWNED</span>
                          <span className="text-sm font-mono text-[var(--text-primary)]">{d.domain}</span>
                        </div>
                        <Button variant="ghost" size="sm" onClick={() => handleRemoveDomain(d.id)} className="text-red-500 hover:text-red-600">✕</Button>
                      </div>
                    ...
                    {showAllDomains && ownedDomains.length > INITIAL_SHOW && (
                      <Button
                        ...
                        className="w-full text-gray-500 hover:text-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                      >
                        折りたたむ ▲
                      </Button>
                    )}
        ...
        {/* Add domain form */}
        <form onSubmit={handleAddDomain} className="flex flex-col sm:flex-row gap-2 mt-4 pt-4 border-t border-[var(--border-subtle)]">
          <input
            type="text"
            value={newDomain}
            onChange={(e) => setNewDomain(e.target.value)}
            placeholder="example.com"
            className="flex-1 border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
          />
          <select
            value={newDomainType}
            onChange={(e) => setNewDomainType(e.target.value as 'primary' | 'owned')}
            className="border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
          >
        ...
        {csvMode && (
          <div className="mt-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 space-y-3">
            <label className="block text-sm font-medium text-[var(--text-primary)]">CSVファイルからドメインを一括登録</label>
            ...
            <div>
              <label className="block text-xs text-[var(--text-secondary)] mb-1">またはCSVテキストを直接入力</label>
              <textarea
                value={csvText}
...
example.net`}
                rows={6}
                className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)] font-mono"
              />
            </div>
      ...
      <div id="section-trademark" className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
        <h2 className="text-sm font-bold text-[var(--text-primary)] mb-4">📄 商標登録証明</h2>
        {brand.trademarkCertUrl ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3 px-4 py-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              ...
              <div className="flex gap-2">
                <a
                  ...
                  className="px-3 py-1.5 bg-white dark:bg-gray-700 border border-[var(--border-default)] text-[var(--text-primary)] rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-600"
                >
                  👁️ プレビュー
                </a>
                <a
            ...
            <div className="flex gap-2">
              <label className="px-3 py-1.5 bg-surface-elevated text-[var(--text-primary)] rounded-lg text-xs hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer">
                🔄 差し替え
              ...
              <Button variant="ghost" size="sm" onClick={handleTrademarkDelete} className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30">
                🗑️ 削除
              </Button>
            </div>
          </div>
          ...
          <div className="text-center py-6 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-lg">
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-3">商標登録証明書をアップロードしてください</p>
            <p className="text-xs text-[var(--text-tertiary)] mb-4">対応形式: PDF, PNG, JPEG（5MB以下）</p>
        ...
        <p className="text-xs text-[var(--text-tertiary)] mt-3">テイクダウン申請時に商標権の証拠として使用されます。</p>
      </div>

      {/* Email Settings */}
      <div id="section-email" className="bg-surface-card rounded-xl border border-[var(--border-default)] p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-bold text-[var(--text-primary)]">
              📧 メール送信設定 <span className="text-xs font-normal text-gray-400">（任意）</span>
            </h2>
            <p className="text-xs text-[var(--text-tertiary)] mt-1">未設定の場合、ThreatGuard共有メールサーバーからテイクダウン通知を送信します</p>
          </div>
          {!showEmailSettings && (
            <Button
              variant="ghost"
              ...
              className="bg-surface-elevated hover:bg-gray-200 dark:hover:bg-gray-600"
            >
        ...
        {!showEmailSettings ? (
          brand.senderEmail ? (
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                <span className="text-[var(--text-secondary)] w-28">送信元メール:</span>
                <span className="text-[var(--text-primary)] font-mono">{brand.senderEmail}</span>
              </div>
              {brand.smtpHost && (
                <div className="flex items-center gap-2">
                  <span className="text-[var(--text-secondary)] w-28">SMTPサーバー:</span>
                  <span className="text-[var(--text-primary)] font-mono">{brand.smtpHost}:{brand.smtpPort || 587}</span>
                </div>
              )}
              {!brand.smtpHost && (
          ...
          <form onSubmit={handleEmailSave} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">送信元メールアドレス</label>
              <input
                type="email"
                className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
            ...
            <div className="border-t border-[var(--border-subtle)] pt-4">
              <p className="text-xs text-[var(--text-secondary)] mb-3">独自SMTPサーバー（任意。未設定の場合は共有メールサーバーを使用）</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">SMTPホスト</label>
                  <input
                    type="text"
                    className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
                ...
                <div>
                  <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">ポート</label>
                  <input
                    type="number"
                    className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
                ...
                <div>
                  <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">ユーザー名</label>
                  <input
                    type="text"
                    className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
                ...
                <div>
                  <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">パスワード</label>
                  <input
                    type="password"
                    className="w-full border border-[var(--border-default)] rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-[var(--text-primary)]"
            ...
            <div className="flex gap-2">
              ...
              <Button variant="secondary" type="button" onClick={() => setShowEmailSettings(false)}>
                キャンセル
              </Button>
            </div>
          </form>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                {scanHistory.scans.map((scan: any) => (
                  <tr key={scan.id}>
                    ...
                    <td className="py-2 text-[var(--text-secondary)]">{new Date(scan.startedAt).toLocaleString('ja-JP')}</td>
                    <td className="py-2 text-[var(--text-secondary)]">{scan.completedAt ? new Date(scan.completedAt).toLocaleString('ja-JP') : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
